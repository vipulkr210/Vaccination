...
<body>
    ...
    <div class="panel panel-primary">
        <div class="panel-heading">
            <h3 class="panel-title panel-primary">Add New Parent </h3>
        </div> 
	...
	if(pid!=null){if(pid.equals("2")){%> <div class="panel-heading" style="background:lightgreen;color:green">
            <h3 class="panel-title panel-primary" >Congratualtions!  You Will Be Able To login to your Parent Login After Approval from your Email ID.<br>Thank you for visiting Vaccination Portal. </h3>
        </div><%}}%> 
		<%
		...
		if(pid!=null){if(pid.equals("4")){%> <div class="panel-heading" style="background:red;color:green">
	            <h3 class="panel-title panel-primary" >Error! Email Alreday Exists. Please Try using New Email Id. </h3>
	        </div><%}}%>
...
<div class="col-md-10">
  	...
  	<form action="parent" method="" onsubmit="vaccination()">
		<table id="ContentPlaceHolder1_DataLstExamSchedule" style="border-collapse:collapse;">
	<input type="hidden" name="t"  readonly="readonly" value="addParent">
<!-- <tr><td>Doctor ID</td><td><input type="text" name="userid" id="userid" readonly="readonly" value=""></td>
</tr> -->
<tr><td>Father Name</td><td><input type="text" name="pName"></td> 
<td>Mother Name</td><td><input type="text" name="mName"></td></tr>
<tr>
...
<td><select name="day"><%for(int i=1;i<=31;i++){ %><option value=<%=i %>><%=i %></option><%} %></select> 
<select name="month"><%for(int i=1;i<=12;i++){ %><option value=<%=i %>><%=i %></option><%} %></select> 
<select name="year"><%for(int i=1980;i<=2025;i++){ %><option value=<%=i %>><%=i %></option><%} %></select> </tr>
<tr><td>Mobile</td><td><input type="text" name="mobile"></td>
<td>Child's Name</td><td><input type="text" name="cname"></td></tr>
...
<option value="West Bengal">West Bengal</option></select>
	...
	<br><br>
 <div class="col-md-6" style="width: 100%;"><input type="submit" name="btnResetPswd" value="Register Parent"  id="btnResetPswd" class="btn btn-raised btn-sm btn-primary btn-block login-btn" data-dismiss="modal" />
                                    </div></center>
 </form>
